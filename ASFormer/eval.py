...
#     print("Acc: %.4f" % (acc))
#     print('Edit: %.4f' % (edit))
    f1s = np.array([0, 0 ,0, 0], dtype=float)
    for s in range(len(overlap)):
...
def main():
    ...
    if args.split == 0:
        for split in range(1, cnt_split_dict[args.dataset] + 1):
            recog_path = f".results/{args.dataset}/split_4/{args.method}/"
            file_list = f"../data/{args.dataset}/splits/test.split4.bundle"
...
#     print("F1@[10, 25, 50, 75]: {}".format(f1s_all))
#     print("Edit: %4f" % (edit_all))
#     print("Acc: %.4f" % acc_all)
    import pandas as pd
#     import pdb; pdb.set_trace()
    f1s_all = f1s_all.tolist()
    f1s_all.append(edit_all)
    f1s_all.append(acc_all)
    cols = ['F1@10', 'F1@25', 'F1@50', 'F1@75', 'Edit', 'Acc']
    result = pd.DataFrame(data = f1s_all, index=cols)
    print(result)
    result.to_csv('./csv/{}_{}.csv'.format(args.dataset, args.method))
