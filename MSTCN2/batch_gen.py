...
class BatchGenerator(object):
    ...
    def next_batch(self, batch_size):
        ...
        for vid in batch:
            ...
            classes = np.zeros(min(np.shape(features)[1], len(content))) # batch중 frame 수가 최소로 통일
            for i in range(len(classes)):
                classes[i] = self.actions_dict[content[i]] # str -> int (encoding)
...
#         K, _ = self.curvature(batch_input_tensor, batch_size, plot=False)
