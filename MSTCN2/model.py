...
class MS_TCN2(nn.Module):
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes):
        ...
        ## SS-TCN의 Dilation layer를 Dual Dilation Layer로 전환 ##
        self.Rs = nn.ModuleList([copy.deepcopy(Refinement(num_layers_R, num_f_maps, num_classes, num_classes)) for s in range(num_R)])

    def forward(self, x):
        out = self.PG(x)  # stage1: generate the initial prediction
        outputs = out.unsqueeze(0) # MS-TCN1

        for R in self.Rs: # 4 Refinement Layers
...
class Refinement(nn.Module): ## SS_TCN이랑 정확히 같은 코드
    def __init__(self, num_layers, num_f_maps, dim, num_classes):
    ...
    def forward(self, x): # MASK만 없고 완전 같은 코드(전체를 다 본다)
        out = self.conv_1x1(x)
...
class SS_TCN(nn.Module):
    def __init__(self, num_layers, num_f_maps, dim, num_classes):
        ...
        self.conv_1x1 = nn.Conv1d(dim, num_f_maps, 1) # adjust the dimension of the input features
        self.layers = nn.ModuleList([copy.deepcopy(DilatedResidualLayer(2 ** i, num_f_maps, num_f_maps)) for i in range(num_layers)]) # 1, 2, 4, ... , 512 -> increase the receptive field without parameter increase
        self.conv_out = nn.Conv1d(num_f_maps, num_classes, 1)
...
class Trainer:
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes, dataset, split, op):
    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate, device):
        ...
        for epoch in range(num_epochs):
            ...
            while batch_gen.has_next():
                ...
                predictions = self.model(batch_input) # MS_TCN2 forward
                ##################
...
#                 import pdb; pdb.set_trace()
                for p in predictions:
                    ...
                    # Trucated mean squared error over the frame-wise log-probabilities 
