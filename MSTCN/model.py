...
class MS_TCN2(nn.Module):
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes):
        ...
        self.PG = PG(num_layers_PG, num_f_maps, dim, num_classes) 
        self.SS_TCNs = nn.ModuleList([copy.deepcopy(SS_TCN(num_layers_R, num_f_maps, num_classes, num_classes)) for s in range(num_R)])

    def forward(self, x):
        out = self.PG(x)  
        outputs = out.unsqueeze(0)

        for R in self.SS_TCNs: 
...
class MS_TCN(nn.Module):
    ...
    def forward(self, x):
        out = self.stage1(x) # stage1: generate the initial prediction
        outputs = out.unsqueeze(0)
